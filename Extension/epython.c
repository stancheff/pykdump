...
static char crashmod_version_s[] = "@(#)pycrash 3.2.1";
...
run_fromzip(const char *progname, const char *zipfilename) {
  ...
  code = (PyObject *) PyObject_CallMethod(importer, "get_code", "s",
                                          progname);
  Py_DECREF(importer);
  if (!code) {
    if (debug)
  ...
  d = PyModule_GetDict(m);
  v =  PyUnicode_FromString(progname);
...
// crash stores all command-line arguments in
// extern char *args[MAXARGS];
// even if they are in Unicode. But Python C-API relies on proper wchar_t
// for arguments. The following subroutine converts (char *) argument array to
// (wchar_t *) array and returns the result. When it is not needed anymore,
// it should freed via PyMem_Free

wchar_t ** argv2wchar(int argc, char *argv[]) {
  wchar_t **w_argv = NULL;
  int i;

  w_argv = (wchar_t **)PyMem_Malloc(sizeof(wchar_t*)*argc);

  for (i = 0; i < argc; i++) {
    w_argv[i] = Py_DecodeLocale(argv[i], NULL);
    // fprintf(stderr, "i=%d allocated at %p\n", i, w_argv[i]);
    if (!w_argv[i])
      PyErr_NoMemory();
  }
...
epython_execute_prog(int argc, char *argv[], int quiet) {
  ...
  // We should add handling exceptions here to prevent 'crash'
  // from exiting
  if (argc > 0) {
    argv_copy = argv2wchar(argc, nargv);
    PySys_SetArgvEx(argc, argv_copy, 0);

    /* The function will be available only on the 2nd and further
       invocations  of epython as it is normally defined in API.py
  ...
  // Free memory allocated for wchar copy
  for (i = 0; i < argc; i++) {
    if (debug) {
      fprintf(stderr, "   %d %p %s freed\n",
              i, argv_copy[i],
              Py_EncodeLocale(argv_copy[i], NULL));
    }
  ...
  if (debug)
    checksignals();
  // Reset sys.path every time after running a program as we
  // do not destroy the intepreter
  PySys_SetPath(wstdpath);
